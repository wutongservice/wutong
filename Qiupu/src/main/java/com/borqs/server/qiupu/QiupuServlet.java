...
//        minSDK=1000;
    ...
    public RecordSet getAllApps(QueryParams qp, HttpServletRequest req) throws AvroRemoteException, UnsupportedEncodingException {
        ...
        if (!qp.getString("cols", "").equals("")) {
            return q.getAllApps(ctx, viewerId, qp.getString("category", "0"), qp.getBoolean("paid", false), qp.getString("sort", "download"), qp.getString("cols", ""), (int) qp.getInt("page", 0), (int) qp.getInt("count", DEFAULT_APP_COUNT_IN_PAGE),qp.getBoolean("history_version", false),minSDK);
        } else {
            return q.getAllAppsFull(ctx, viewerId, qp.getString("category", "0"), qp.getBoolean("paid", false), qp.getString("sort", "download"), (int) qp.getInt("page", 0), (int) qp.getInt("count", DEFAULT_APP_COUNT_IN_PAGE),qp.getBoolean("history_version", false),minSDK);
        }
    }
    ...
    public RecordSet suggestApps(QueryParams qp,HttpServletRequest req) throws ResponseError, Exception {
        ...
        String ua = getDecodeHeader(req, "User-Agent", "");
        int minSDK =  getMinSDKFromUA(ua);
        return qiupu.suggestApks(ctx, viewerId, category, cols, page, count,ua,minSDK);
    }
