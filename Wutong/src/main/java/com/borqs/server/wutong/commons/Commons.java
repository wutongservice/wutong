...
import com.borqs.server.base.util.PhoneNumberHelper;
...
public class Commons {
    ...
    public String SERVER_HOST = "api.borqs.com";
            ...
            if (value.equals("0")) {
                ...
                for (String email : emails) {
                    if (StringUtils.isNotBlank(email) && !haveSend.contains(email)) {
                        GlobalLogics.getEmail().sendEmail(ctx,title, email, email, emailContent, emailType, lang);
                        haveSend.add(email);
                    }
                }
            }
...
//                }
    ...
    public String getOldMentions(Context ctx, String userId, String fromMentions) {
    ...
    public String composeShareContent(Context ctx, String viewerId, int type, String body, boolean isEmail, String device) {
        String displayName = GlobalLogics.getAccount().getUser(ctx, viewerId, viewerId, "display_name", false).getString("display_name");
        String shareType = "";
        switch (type) {
...
//                throw new BaseException(ErrorCode.DATA_ERROR, "file size is too large");
    ...
    public static Record formatFileBucketUrl(Context ctx,String viewerId, Record rec) throws AvroRemoteException {
    ...
    public static Record formatFileBucketUrlForStream(Context ctx,String viewerId, Record rec) {
        ...
        if (!rec.isEmpty()) {
            ...
            if (content_type.contains("/video")) {
                ...
                Record rec_video = folderLogic.getVideoById(ctx,rec.getString("file_id"));
