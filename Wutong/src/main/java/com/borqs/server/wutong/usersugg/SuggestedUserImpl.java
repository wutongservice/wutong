...
//            RecordSet rs=  RecordSet.fromByteBuffer(su.getSuggestUser(userId, limit));
        ...
        if (rs.size() > 0) {
            for (int i = 0; i < rs.size(); i++) {
                ...
                Record u = account.getUsers(ctx, userId, suggestedId, "user_id, display_name, image_url,remark,perhaps_name").getFirstRecord();
                if (!u.isEmpty()) {
                    ...
                    if (rs.get(i).getInt("type") == Long.parseLong(RECOMMENDER_USER)
                        ...
                        if (reasonStr.length() > 0) {
                            RecordSet reasonUser = account.getUsers(ctx, reasonStr, "user_id, display_name, image_url,remark,perhaps_name");
