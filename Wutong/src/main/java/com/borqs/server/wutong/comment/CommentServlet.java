...
public class CommentServlet extends WebMethodServlet {
    ...
    public Record createComment(QueryParams qp, HttpServletRequest req) throws AvroRemoteException, UnsupportedEncodingException {
        ...
        return GlobalLogics.getComment().createCommentP(ctx,viewerId, (int) qp.getInt("object", 2), qp.checkGetString("target"), qp.checkGetString("message"), ua, can_like, loc,add_to, appId);
    }
    ...
    public RecordSet getCommentsFor(QueryParams qp) throws AvroRemoteException {
        ...
        if (qp.getString("comments", "").isEmpty() || qp.getString("comments", "").equals("")) {
            return GlobalLogics.getComment().getFullCommentsForP(ctx,viewerId,(int) qp.getInt("object", 2), qp.getString("target", ""), qp.getBoolean("asc", false), (int) qp.getInt("page", 0), (int) qp.getInt("count", 20));
        } else {
            return GlobalLogics.getComment().getCommentsForP(ctx,viewerId,(int) qp.getInt("object", 2), qp.getString("target", ""), qp.checkGetString("comments"), qp.getBoolean("asc", false), (int) qp.getInt("page", 0), (int) qp.getInt("count", 20));
        }
    }
