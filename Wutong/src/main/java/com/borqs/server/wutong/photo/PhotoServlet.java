...
public class PhotoServlet extends WebMethodServlet {
    ...
    public RecordSet getPhotoByIds(QueryParams qp, HttpServletResponse resp) {
        ...
        Configuration conf = getConfiguration();
        for (Record rec : recs) {
            rec = formatPhotoUrlAndExtend(ctx, viewerId, rec, conf);
        }
        return recs;
    }
    ...
    public Record tagPhoto(QueryParams qp) {
        ...
        Configuration conf = getConfiguration();
        record = formatPhotoUrlAndExtend(ctx, viewerId, record, conf);
        return record;
    }
    ...
    public Record formatPhotoUrlAndExtend(Context ctx, String viewerId, Record rec, Configuration conf) {
    ...
    public Record sharePhoto(QueryParams qp, HttpServletRequest req) {
        ...
        if (fi != null && StringUtils.isNotEmpty(fi.getName())) {
            ...
            if (result) {            // && !album.getBoolean("privacy",false)
                ...
                post_id = streamLogic.postP(ctx,userId, Constants.PHOTO_POST, msg, sRecord.toString(false, false), qp.getString("appid", "1"),
