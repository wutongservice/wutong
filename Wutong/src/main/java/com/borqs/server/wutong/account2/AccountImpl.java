        ...
        try {
            ...
            if (l2.contains("friends_count")) {
                for (Record rec : recs) {
                    ...
                    int shared_text = getSharedCount(ctx, viewerId, rec.getString("user_id"), Constants.TEXT_POST);
                    shared_count.put("shared_text", shared_text);
                    int shared_photo = getSharedCount(ctx, viewerId, rec.getString("user_id"), Constants.PHOTO_POST);
                    shared_count.put("shared_photo", shared_photo);
                    int shared_book = getSharedCount(ctx, viewerId, rec.getString("user_id"), Constants.BOOK_POST);
                    shared_count.put("shared_book", shared_book);
                    int shared_apk = getSharedCount(ctx, viewerId, rec.getString("user_id"), Constants.APK_POST);
                    shared_count.put("shared_apk", shared_apk);
                    int shared_link = getSharedCount(ctx, viewerId, rec.getString("user_id"), Constants.LINK_POST);
                    shared_count.put("shared_link", shared_link);
                    int shared_static_file = getSharedCount(ctx, viewerId, rec.getString("user_id"), Constants.FILE_POST);
                    shared_count.put("shared_static_file", shared_static_file);
                    int shared_audio = getSharedCount(ctx, viewerId, rec.getString("user_id"), Constants.AUDIO_POST);
                    shared_count.put("shared_audio", shared_audio);
                    int shared_video = getSharedCount(ctx, viewerId, rec.getString("user_id"), Constants.VIDEO_POST);
...
//                RecordSet posts = getFullPostsForQiuPu(user.getString("user_id"), topPostIds, true);
    ...
    public String createAccount(Context ctx, String login_email1, String login_phone1, String pwd,
        ...
        streamLogic.autoPost(ctx, userId, Constants.TEXT_POST, message, tempNowAttachments, toStr(appid), "", "", m, "", false, "", device, location, true, true, true, "", "", false);
...
//        }
        ...
        if (StringUtils.isBlank(key)) {
            ...
            for (String email : emails) {
                if (StringUtils.isNotBlank(email)) {
                    emailLogic.sendEmail(ctx, title, email, email, content, Constants.EMAIL_ESSENTIAL, lang);
                }
            }
        } else {
            ...
            for (String email : emails) {
                if (StringUtils.isNotBlank(email)) {
                    emailLogic.sendEmail(ctx, Constants.getBundleStringByLang(lang, "platform.reset.password.msgtitle"), email, email, content, Constants.EMAIL_ESSENTIAL, lang);
                }
            }
        }
        if (L.isTraceEnabled())
    ...
    public Record updateUserStatus(Context ctx, String userId, String newStatus, String device, String location, boolean post, boolean can_comment, boolean can_like, boolean can_reshare) {
        ...
        if (b) {
            if (post) {
                String postid = streamLogic.postP(ctx, userId, Constants.TEXT_POST, newStatus, "[]", toStr(Constants.APP_TYPE_QIUPU), "", "", "", "", false, "", device, location, "", "", can_comment, can_like, can_reshare, "");
