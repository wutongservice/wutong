...
public class CommentImpl implements CommentLogic, Initializable {
    ...
    public Record createCommentP(Context ctx, String userId, int objectType, String target, String message, String device, Boolean canLike, String location, String add_to, String appId) {
        ...
        if (objectType == Constants.APK_OBJECT) {
            ...
            GlobalLogics.getStream().autoPost(ctx, userId, Constants.APK_COMMENT_POST, message, tempNowAttachments, String.valueOf(Constants.APP_TYPE_QIUPU), "", target, m, "", false, Constants.QAPK_FULL_COLUMNS, device, location, true, true, true, "", "", false);
...
//                sendCommentOrLikeEmail(Constants.APK_OBJECT, userId, user, target, message, lang);
    ...
    public Record createComment1P(Context ctx, String userId, int objectType, String target, String message, String device, String location, String add_to, String appId) {
        return createCommentP(ctx, userId, objectType, target, message, device, true, location, add_to, appId);
    }
    ...
    public RecordSet transComment(Context ctx, String viewerId, RecordSet commentDs) {
        ...
        if (commentDs.size() > 0) {
            ...
            for (Record r : commentDs) {
                r.put("image_url",GlobalLogics.getAccount().getUser(ctx,r.getString("commenter"),r.getString("commenter"),"image_url").getString("image_url"));
